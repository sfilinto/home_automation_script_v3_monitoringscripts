...
usage ()
{
	if [ $# -le 1 ]
	then
		echo "Usage: $(basename $0) <-u program_when_ip_comes_online> <-d program_when_ip_goes_offline> [-e] <hostname_or_IP_address>"
		echo "       by default, notifications are level triggered. -e makes them edge triggered."
		echo "       eg. ./monitorip.sh -u ./up.sh -d ./down.sh -e 192.168.1.2"
		exit 1
	fi
	exit 0
}
...
case $key in
    ...
    #echo down:$2
    EDGE_TIGGERED=1
    echo 'Notifications will be edge triggered'
    # TODO: do this initialization later
    # what happens when -e comes before -u and -d
    ONLINE_CALL_EDGE=$ONLINE_CALL
    OFFLINE_CALL_EDGE=$OFFLINE_CALL
    # reset the level triggered params
    ONLINE_CALL=""
    OFFLINE_CALL=""
    ;;
    --default)
